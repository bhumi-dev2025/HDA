...
export default function ChatScreen() {
  ...
  const executeSingleToolCall = async (functionName: string, args: Record<string, any>): Promise<string | null> => {

    // ── ADD TODO ──────────────────────────────────────────────────────────────
    if (functionName === "add_todo") {
      const newTasks: { text: string }[] = args.tasks ?? [];
      if (newTasks.length === 0) return "I couldn't find any tasks to add. Please mention what you want to add.";
      try {
        ...
        const remaining = result.merged.filter((m: any) => m.text.trim() !== "").length;
    ...
    // ── REMOVE TODO ───────────────────────────────────────────────────────────
      ...
      if (!removeAll && tasksToRemove.length === 0) return "Please tell me which task you want to remove.";
      try {
        ...
        if (result.removedCount === 0) return "I couldn't find that task. Please check the task name.";
    ...
    // ── SET MEDITATION ────────────────────────────────────────────────────────
      ...
      try {
        ...
        if (!result.success) return "Could not update meditation. Please try again.";
    ...
    // ── SET WATER ─────────────────────────────────────────────────────────────
      ...
      try {
        ...
        if (!result.success) return "Could not update water intake. Please try again.";
    ...
    // ── SET SLEEP ─────────────────────────────────────────────────────────────
      ...
      const sleepData = { hour: hour.toString().padStart(2, "0"), minute: minute.toString().padStart(2, "0") };
      try {
    ...
    // ── SET WORKOUT ───────────────────────────────────────────────────────────
      ...
      const workoutData = { hour: hour.toString().padStart(2, "0"), minute: minute.toString().padStart(2, "0") };
      try {
        const result = await updateDailyLog("workout", workoutData);
        if (!result.success) return "Could not update workout. Please try again.";
  ...
  const handleMultiToolCall = async (calls: { functionName: string; args: Record<string, any> }[]) => {
  ...
  const handleSend = async () => {
    ...
    if (!hasKey) {
      Alert.alert("No API Key", "Please add your Gemini API key in Settings first.",
        [{ text: "Go to Settings", onPress: () => router.push("/settings/ai") }]);
      return;
    }
    ...
    try {
      ...
      Alert.alert(title, message, [{ text: isNoKey ? "Go to Settings" : "OK", onPress: isNoKey ? () => router.push("/settings/ai") : undefined }]);
    } finally {
      setLoading(false);
    }
  };
