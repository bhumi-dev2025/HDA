...
import M2 from "../../../assets/2.0/model/M2.svg";
import M21 from "../../../assets/2.0/model/M21.svg";
import M22 from "../../../assets/2.0/model/M22.svg";
...
const toTD = (m: number) => ({
  hour: String(Math.floor(m / 60)).padStart(2, "0"),
  minute: String(m % 60).padStart(2, "0"),
});
const fromTD = (t: { hour: string; minute: string }) =>
  parseInt(t.hour || "0") * 60 + parseInt(t.minute || "0");

type TimeData = { hour: string; minute: string };
type Props = {
  isVisible: boolean;
  onClose: () => void;
  onSave: (v: TimeData) => void;
  initialValue?: TimeData;
};
...
export function WorkoutBottomSheet({
  ...
  // Center button — left slide out (exact reference: translateX by ITEM_H + gap)
  ...
  // Block area — fades + scales out (exact reference)
  ...
  // Slider — fades + scales in from right (exact reference)
  ...
  const handleSave = () => {
    const mins = STEPS[Math.max(0, Math.min(stepIdx.get(), STEPS.length - 1))];
    onSave(toTD(mins));
    onClose();
  };
  ...
  return (
    <AppBottomSheet isVisible={isVisible} onClose={onClose}>
      <View style={s.container}>
        ...
        <Text style={s.desc}>
          Monitor your workout sessions to gain deeper insights into your
          fitness journey.
        </Text>
        ...
        <View style={s.timerRow}>
          ...
          <Animated.View style={aStepperBtn}>
            <TouchableOpacity
              ...
              style={s.iconBtn}
            >
              <M21 width={ITEM_H} height={ITEM_H} />
            </TouchableOpacity>
          </Animated.View>

          {/* Center display — left slide out on tap (exact reference) */}
          ...
          <Animated.View style={aStepperBtn}>
            <TouchableOpacity
              ...
              style={s.iconBtn}
            >
              <M22 width={ITEM_H} height={ITEM_H} />
            </TouchableOpacity>
          </Animated.View>

          {/* Block area — fades out when slider opens */}
          ...
          {/* Slider — absolute, right side thi aave */}
